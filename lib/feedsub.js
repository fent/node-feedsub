  ...
  var req = this.r.get(this.getOpts, function(res) {
    ...
    var firstitem = function(item) {
      ...
      if (self.options.skipHours || self.options.skipDays) {
        var now = new Date();
        if ((self.options.hours && self.options.hours.indexOf(now.getHours()) !== -1) || (self.options.days && self.options.days.indexOf(DAYS[now.getDay()]) !== -1)) {
          req.abort();
    ...
    var getitems = function(item) {
      ...
      if (self.options.history.length > 0) {
        var iterator = function(hitem) {
          return _.isEqual(hitem.title, item.title) && ((!hitem.pubdate && !hitem.published) || (hitem.pubdate && hitem.pubdate === item.pubdate) || (hitem.published && hitem.published === item.published));
        };
